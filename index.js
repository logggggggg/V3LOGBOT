	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔ Nice mlk, deu certo ✔',
				error: {
					isOff: 'Bot está desativado temporáriamente, tente novamente mais tarde',
					stick: '⚠ Error: não foi possivel converter imagem em figurinha ⚠',
					Iv: ' Link invalido ',
					off: ' Comando Desativado'
				},
				only: {
					group: ' Desculpe, esse comando só pode ser usado em grupos',
					ownerG: ' KKK, só o dono pode usar isso ',
					ownerB: ' KKK, só o dono pode usar isso ',
					admin: ' Só adm pode usar esse comando',
					Badmin: ' O Bot não é adm',
					pv: '*SOMENTE NO PV DO BOT*'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["553898456180@s.whatsapp.net", "] // replace this with your number"]
			...
			switch (command) {
			   ...
			   case 'bot':
				...
				case 'promote':
					...
					if (mentioned.length < 2) {
						...
						client.sendMessage(from, `Esse cara aqui @${mentioned[0].split('@')[0]} agr é adm...`, text)
					}
				...
				case 'demote':
					...
					if (mentioned.length < 2) {
						...
						client.sendMessage(from, `Esse cara aqui @${mentioned[0].split('@')[0]} agr não é mais adm...`, text)
					}
					else if (mentioned.length >= 2) {
						client.sendMessage(from, "1 pessoa de cada vez", text)
					}
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length < 2) {
						client.sendMessage(from, `@${mentioned[0].split('@')[0]} o otario levou ban KKKKKKKK`, text)
				...
				case 'bug':
					...
					var gpa = '553898456180-1614654837@g.us'
                ...
                case 'ytmsc':
                   ...
                   client.sendMessage(from, "LOG BOT BRABOKLKK", text, {quoted: mek})
                   await limitAdd(sender) 
                   break	   
               ...
               case 'sn':
					...
					client.sendMessage(from, `~*LOG BOT RESPONDE*~: \n\n_Pergunta: ${perg}_ \n*Resposta: ${resp}* \n\n 😎🤙`, text, { quoted: mek })
					break
